...
void central_conn_query_handler(comm_param_t* p_context);
...
action_table_t cmd_list[CMD_LIST_SIZE] = {
	...
	{"CENTRAL_CONN_QUERY", central_conn_query_handler},
...
void central_conn_query_handler(comm_param_t* p_context)
{
	if (BLE_CONN_HANDLE_INVALID == g_central_conn_handle)
	{
		cmd_general_rsp(p_context, "invalid", strlen("invalid"));
	}
	else
	{
		cmd_general_rsp(p_context, "connect", strlen("connect"));
	}
}
